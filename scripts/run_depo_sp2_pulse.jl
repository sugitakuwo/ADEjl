...
#Utility Functions to generate pulse input
module Utils
...
#---Blocking function based on the RSA model for non-spherical particles
...
module ADEmain
    export ADE_depo_RSA_SP2_pulse, ADE_depo_RSA_spherocyl2_pulse
...
xs, ts, solc, soltheta, sol1 = ADE_depo_RSA_SP2_pulse(L=Lc,xmax=xmax,tmax=tmax,vp_in=vp,D_in=D,f_in=f,kdepo_in=kdepo,Areap=Area_Cyl,C0_in=C0,theta_max=thmax,Aspr=As,pulse_width=pulse_width_in,dx=dx_in,theta_in=theta,x_in=xsol,solver=solverin)
...
function fit_ADEdepo_SP2_pulse()
date=Dates.today()
...
file2=fn[2]
file3=fn[3]
Absinput=0.3468#Abs for input cnc
...
C0=2*7.323e19#6.58e19# Particle initial number concentration [1/m3]
...
exp1[:,1]=(exp1[:,1].-(tatmax2nd-tp))/tp
exp2[:,1]=(exp2[:,1].-(tatmax2nd-tp))/tp
exp3[:,1]=(exp3[:,1].-(tatmax2nd-tp))/tp
...
lower = [2.0e-8,0.01]#Should change lower/upper bounds for each parameter?
upper = [2.0e-6,0.5467]#
p = [1,1]
n_particles = 5#3, 5, 7
options = Optim.Options(iterations=50)#20, 50, 70
go_optim = false #true / false
kdepo_test = 8.9e-7#2.0e-6#5.0e-7
thmax_test = 0.14#0.06842063432190323#0.4
...
#sol1 = solve(prob1, ParticleSwarm())
    res1 = Optim.optimize(p -> Lsq_ADEdepo_pulse(p,exp1,Lc,xmax,tmax,vp,D,f,Area_ave,C0,Aspr,dx_in,pulse_width,solverin), initial_x, ParticleSwarm(lower,upper,n_particles),options)
...
#2nd pulse
    xs, ts, solc, soltheta, sol1 = ADE_depo_RSA_SP2_pulse(L=Lc,xmax=xmax,tmax=tmax,vp_in=vp,D_in=D,f_in=f,kdepo_in=initial_x[1],Areap=Area_ave,C0_in=C0,theta_max=initial_x[2],pulse_width=pulse_width,dx=dx_in,solver=solverin)
    res2 = Optim.optimize(p -> Lsq_ADEdepo_pulse(p,exp2,Lc,xmax,tmax,vp,D,f,Area_ave,C0,Aspr,dx_in,pulse_width,solverin,theta=soltheta,xsol=xs), initial_x, ParticleSwarm(lower,upper,n_particles),options)
    println("Optimized (kdepo, thmax) for 2nd pulse =",Optim.minimizer(res2)[1]," ",Optim.minimizer(res2)[2]," and initial guess = ", initial_x)
#3rd pulse
    xs2, ts2, solc2, soltheta2, sol2 = ADE_depo_RSA_SP2_pulse(L=Lc,xmax=xmax,tmax=tmax,vp_in=vp,D_in=D,f_in=f,kdepo_in=initial_x[1],Areap=Area_ave,C0_in=C0,theta_max=initial_x[2],pulse_width=pulse_width,dx=dx_in,theta_in=soltheta,x_in=xs,solver=solverin)
    res3 = Optim.optimize(p -> Lsq_ADEdepo_pulse(p,exp3,Lc,xmax,tmax,vp,D,f,Area_ave,C0,Aspr,dx_in,pulse_width,solverin,theta=soltheta2,xsol=xs2), initial_x, ParticleSwarm(lower,upper,n_particles),options)
    println("Optimized (kdepo, thmax) for 3rd pulse =",Optim.minimizer(res2)[1]," ",Optim.minimizer(res2)[2]," and initial guess = ", initial_x)
...
xs, ts, solc, soltheta, sol1 = ADE_depo_RSA_SP2_pulse(L=Lc,xmax=xmax,tmax=tmax,vp_in=vp,D_in=D,f_in=f,kdepo_in=kdepo,Areap=Area_ave,C0_in=C0,theta_max=thmax,pulse_width=tpulse/tp,dx=dx_in,solver=solverin)
xs2, ts2, solc2, soltheta2, sol2 = ADE_depo_RSA_SP2_pulse(L=Lc,xmax=xmax,tmax=tmax,vp_in=vp,D_in=D,f_in=f,kdepo_in=kdepo,Areap=Area_ave,C0_in=C0,theta_max=thmax,pulse_width=tpulse/tp,dx=dx_in,theta_in=soltheta,x_in=xs,solver=solverin)
xs3, ts3, solc3, soltheta3, sol3 = ADE_depo_RSA_SP2_pulse(L=Lc,xmax=xmax,tmax=tmax,vp_in=vp,D_in=D,f_in=f,kdepo_in=kdepo,Areap=Area_ave,C0_in=C0,theta_max=thmax,pulse_width=tpulse/tp,dx=dx_in,theta_in=soltheta2,x_in=xs2,solver=solverin)
...
savefig("ADEdepo_pulse_kdepo$(round(kdepo*1e7,digits=6))e-7_thmax$(round(thmax,digits=6))_$(date).png")
